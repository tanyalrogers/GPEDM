...
The timesteps (each row in your data table) are *evenly spaced* and *in chronological order*, otherwise the automatic lag construction will not work properly.
...
Our example dataset will be the chaotic 3-species Hastings-Powell model. There are simulated data from this model included in the `GPEDM` package. We will add a bit of observation noise to the data (Fig. \ref{fig:dataplot}).

```{r dataplot, fig.width=5, fig.height=4, fig.cap="\\label{fig:dataplot} Time series from the Hastings-Powell model with added observation noise."}
head(HastPow3sp)
n=nrow(HastPow3sp)
set.seed(1)
...
```{r gpplot, fig.width=7, fig.height=3, fig.cap="\\label{fig:gpplot} Observed and predicted values from a GP model."}
summary(gp_out1)
...
```{r predict, fig.width=7, fig.height=3, fig.cap="\\label{fig:gpplot} Observed and predicted values from a GP model for a second test dataset."}
...
```{r gp2a, fig.width=7, fig.height=3, fig.cap="\\label{fig:gp2a} Observed and predicted values from a GP model. This uses the Custom Lags Method."}
...
```{r gp2b, fig.width=7, fig.height=3, fig.cap="\\label{fig:gp2b} Observed and predicted values from a GP model. This uses the Custom Lags Method and leave-one-out."}
...
```{r data, fig.width=7, fig.height=3, fig.cap="\\label{fig:data} Time series from two simulated populations with theta logistic dynamics."}
head(thetalog2pop)
...
```{r hierplot, fig.cap="\\label{fig:hierplot} Observed and predicted values from a 2 population hierarchical GP model."}
plot(tlogtest)
```
...
```{r conditionals1, fig.width=7, fig.height=3, fig.cap="\\label{fig:conditionals1} Conditional responses to each predictor in the multivariate model for the Hastings-Powell simulation."}
#From the multivariate Hastings-Powell simulation
getconditionals(gp_out_mv)
```

```{r conditionals2, fig.cap="\\label{fig:conditionals1} Conditional responses to each predictor in the model for the 2 population theta logistic simulation. Recall that the phi values for lags 2 and 3 were 0, so the corresponding relationship is flat."}
...
```{r gpgrad, fig.cap="\\label{fig:gpgrad} Local slope coefficients from the multivariate GP model."}
#From the multivariate Hastings-Powell simulation
