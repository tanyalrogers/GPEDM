...
In `newdata`, only the first timepoint (row) for `m` and `h` needs to be filled in. The rest can be NA and will be filled in as the model iterates forward. The values of `time` and must be filled in. If there are covariates (`z`), they should also be included in `newdata`, and values must be supplied for all time points. If there are multiple populations, `newdata` should contain `pop` and there should be duplicated timesteps for each population (see the next section on general iterated predictions for a 2-population example).
